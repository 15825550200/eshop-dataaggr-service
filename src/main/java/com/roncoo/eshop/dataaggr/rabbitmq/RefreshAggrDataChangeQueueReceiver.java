...
public class RefreshAggrDataChangeQueueReceiver {
	...
	private void processProductDimDataChange(JSONObject messageJSONObject) {
		...
		if (productDataJSON!=null && !"".equals(productDataJSON)) {
			JSONObject productDataJSONObiect = JSONObject.parseObject(productDataJSON);
			...
			if (productPropertyDataJSON!=null && !"".equals(productPropertyDataJSON)) {
				//parse转化为object而不是jsonobject
				productDataJSONObiect.put("product_property", JSONObject.parse(productPropertyDataJSON));
			}
			...
			if (productSpecificationDataJSON!=null && !"".equals(productSpecificationDataJSON)) {
				productDataJSONObiect.put("product_specification", JSONObject.parse(productSpecificationDataJSON));
			}
			jedis.set("dim_product_"+id, productDataJSONObiect.toJSONString());
		} else {
